...
class SemanticPairsDataset(Dataset):
    ...
    def _ordered_pair(self, idx1: int, idx2: int) -> Dict:
        ...
        if cos1 >= cos2:
            first_idx, second_idx = idx1, idx2
            first_embedding, second_embedding = emb1, emb2
            first_norm, second_norm = norm1, norm2
        else:
            first_idx, second_idx = idx2, idx1
            first_embedding, second_embedding = emb2, emb1
            first_norm, second_norm = norm2, norm1

        avg_embedding = 0.5 * (first_embedding + second_embedding)
        pair_embedding = torch.cat([first_embedding, second_embedding], dim=0)

        return {
            "embedding_1": first_embedding,
            "embedding_2": second_embedding,
            "pair_embedding": pair_embedding,
            "avg_embedding": avg_embedding,
            # Backward-compatible aliases with the current semantic code.
            "dominant_embedding": first_embedding,
            "recessive_embedding": second_embedding,
            "embedding_1_idx": first_idx,
            "embedding_2_idx": second_idx,
            "dominant_idx": first_idx,
            "recessive_idx": second_idx,
            "embedding_1_sample_id": self.sample_ids[first_idx],
            "embedding_2_sample_id": self.sample_ids[second_idx],
            "dominant_sample_id": self.sample_ids[first_idx],
            "recessive_sample_id": self.sample_ids[second_idx],
            "embedding_1_source_path": self.source_paths[first_idx],
            "embedding_2_source_path": self.source_paths[second_idx],
            "dominant_source_path": self.source_paths[first_idx],
            "recessive_source_path": self.source_paths[second_idx],
            "embedding_1_relative_path": self.relative_paths[first_idx],
            "embedding_2_relative_path": self.relative_paths[second_idx],
            "dominant_relative_path": self.relative_paths[first_idx],
            "recessive_relative_path": self.relative_paths[second_idx],
            "embedding_1_norm": first_norm,
            "embedding_2_norm": second_norm,
            "dominant_norm": first_norm,
            "recessive_norm": second_norm,
        }
...
def collate_semantic_pairs(batch: List[Dict]) -> Dict:
    embedding_1 = torch.stack([item["embedding_1"] for item in batch], dim=0)
    embedding_2 = torch.stack([item["embedding_2"] for item in batch], dim=0)
    pair_embedding = torch.stack([item["pair_embedding"] for item in batch], dim=0)
    avg_embedding = torch.stack([item["avg_embedding"] for item in batch], dim=0)
    ...
    return {
        ...
        "dominant_embedding": embedding_1,
        "recessive_embedding": embedding_2,
        "embedding_1_idx": torch.tensor([item["embedding_1_idx"] for item in batch], dtype=torch.long),
        "embedding_2_idx": torch.tensor([item["embedding_2_idx"] for item in batch], dtype=torch.long),
        ...
        "embedding_1_norm": torch.tensor([item["embedding_1_norm"] for item in batch], dtype=torch.float32),
        "embedding_2_norm": torch.tensor([item["embedding_2_norm"] for item in batch], dtype=torch.float32),
        ...
        "embedding_1_sample_id": [item["embedding_1_sample_id"] for item in batch],
        "embedding_2_sample_id": [item["embedding_2_sample_id"] for item in batch],
        ...
        "embedding_1_source_path": [item["embedding_1_source_path"] for item in batch],
        "embedding_2_source_path": [item["embedding_2_source_path"] for item in batch],
        ...
        "embedding_1_relative_path": [item["embedding_1_relative_path"] for item in batch],
        "embedding_2_relative_path": [item["embedding_2_relative_path"] for item in batch],
