...
def get_unet(image_size):
    ...
    return UNet2DModel(
        ...
        in_channels=3,
        out_channels=6,  # This version predicts both images.
        layers_per_block=2,
...
def sample_and_capture_xt(model, diffusion, mixed_image, alpha_init, capture_timesteps):
    """
    Reproduces the iterative sampling function from the 6-channel model version.

    The UNet predicts 6 channels, corresponding to bright and dark predictions.
    However, just like in the training script's sampling function, only the first
    3 channels are used as the bright prediction, and the dark image is recovered
    algebraically from the mixture.
    """
    n = len(mixed_image)
    ...
    for i in reversed(range(1, init_timestep + 1)):
        ...
        # Same as the provided script: use only the bright prediction.
        predicted_bright = model_out[:, :3]

        # Same algebraic extraction used during sampling.
        predicted_dark = (
            mixed_image - math.sqrt(1.0 - alpha_init) * predicted_bright
        ) / math.sqrt(alpha_init)

        # Same clamping logic as the provided script.
...
def main(args):
    ...
    # That sample corresponds to the first row collected for eval_grid_50.jpg.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "This version is for the 6-channel UNet that predicts bright and dark images."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to experiments/<run_name>/samples/eval_first_row_separate_6ch"
        ),
    )

    # Arguments kept compatible with get_data(args, 'val').
    parser.add_argument("--test_images", default=1000, type=int)
    parser.add_argument("--train_samples_per_epoch", default=30000, type=int)
    ...
    if args.output_dir is None:
        args.output_dir = os.path.join(
            "experiments", args.run_name, "samples", "eval_first_row_separate_6ch"
        )
